 ...
 *
 * @class H5P.CurrikiInteractiveVideoInteraction
 * @extends H5P.EventDispatcher
 * @param {Parameters} parameters describes action behavior
 * @param {H5P.CurrikiInteractiveVideo} player instance
...
function Interaction(parameters, player, previousState) {
  ...
  var createButton = function (preventAnimation) {
    ...
    if (self.getRequiresCompletion() &&
        ...
        player.currentState !== H5P.CurrikiInteractiveVideo.SEEKING) {
      openDialog(true);
    }
   ...
   *
   * @return {Array.<H5P.CurrikiInteractiveVideoInteraction>}
  ...
  self.getCopyrights = function () {
    if (!self.isStandaloneLabel()) {
      ...
      if (instance !== undefined) {
        ...
        interactionCopyrights.setLabel(title + ' ' + H5P.CurrikiInteractiveVideo.humanizeTime(parameters.duration.from) + ' - ' + H5P.CurrikiInteractiveVideo.humanizeTime(parameters.duration.to));
  ...
  self.getClipboardData = function () {
    return H5P.DragNBar.clipboardify(H5PEditor.CurrikiInteractiveVideo.clipboardKey, parameters, 'action');
  };
